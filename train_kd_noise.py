...
parser.add_argument('--save_path', default='experiments/CIFAR10/kd_normal_resnet18/cnn_backup', type=str)
...
# ************************** training function **************************
def train_epoch_kd(model, t_model, optim, loss_fn_kd, data_loader, params, noise):
    model.train()
    t_model.eval()
    ...
    with tqdm(total=len(data_loader)) as t:  # Use tqdm for progress bar
        for i, (train_batch, labels_batch) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                noisy_output_teacher_batch = t_model(noisy_train_batch)   # logit without SoftMax
            ...
            loss = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(output_batch/T, dim=1),
                             F.softmax(noisy_output_teacher_batch/T, dim=1)) * (params.alpha * T * T) + \
            ...
            output_batch = np.argmax(output_batch.detach().cpu().numpy(), axis=1)
    
    return loss_avg()


def evaluate(model, loss_fn, data_loader, params, noise, is_teacher=False):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for data_batch, labels_batch in data_loader:
            ...
            lamb=1.0
            noisy_data_batch = lamb * data_batch + \
                                (1-lamb) * noise.unsqueeze(0).repeat(data_batch.size()[0],1,1,1)
            # compute model output
            output_batch = model(noisy_data_batch)
    ...
    if is_teacher==False:
        s_test_acc.append((class_correct.float()/class_batch_num).cpu().numpy())
    else:
        t_test_acc.append((class_correct.float()/class_batch_num).cpu().numpy())
        for i in range(class_num):
            print('Class['+class_name[i]+']:',' '*(10-len(class_name[i])), str(t_test_acc[-1][i]))
...
def train_and_eval_kd(model, t_model, optim, loss_fn, train_loader, dev_loader, params):
    ...
    noise_data = torch.load(os.path.join(args.save_path, 'noise_data.pth'))
    if params.cuda:
    ...
    for epoch in range(params.num_epochs):
        ...
        train_loss = train_epoch_kd(model, t_model, optim, loss_fn, train_loader, params, noise = noise_data)
        logging.info("- Train loss : {:05.3f}".format(train_loss))

        # ********************* Evaluate for one epoch on validation set *********************
        val_metrics = evaluate(model, nn.CrossEntropyLoss(), dev_loader, params, noise = noise_data)  # {'acc':acc, 'loss':loss}
        ...
        if (epoch+1)%10==0 or epoch==0:
            ...
            pd_data = pd.DataFrame(columns=class_name, data=t_test_acc)
            pd_data.to_csv(os.path.join(args.save_path, 'teacher_test_acc.csv'), encoding='gbk')
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(teacher_resume, map_location='cuda:'+str(device_ids[0]))
    ...
    val_metrics = evaluate(teacher_model, nn.CrossEntropyLoss(), devloader, params, noise=noise_data, is_teacher=True)  # {'acc':acc, 'loss':loss}
    metrics_string = " ; ".join("{}: {:05.3f}".format(k, v) for k, v in val_metrics.items())
